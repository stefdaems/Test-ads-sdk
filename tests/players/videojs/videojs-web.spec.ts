/**
 * Video.js — Web Integration Tests
 *
 * Exercises the full Ads SDK feature matrix against Video.js running in
 * Chromium via Playwright.
 *
 * Prerequisites:
 *   Start the Video.js host page server:
 *     cd host-apps/web/players/videojs && npx serve . --listen 3002
 */
...
beforeEach(async () => {
  clearHar();
  page = await newPage(browser, 'videojs');
});
...
describe('Video.js Web — Initialization', () => {
  ...
  test('Video.js IMA adapter attaches without error', async () => {
    const log = await readDiagnosticLog(page);
    expect(log).toContain('adapter_attached:videojs');
  });

  test('SDK rejects null publisher ID gracefully', async () => {
    const badPage = await newPage(browser, 'videojs', 'bad');
...
describe('Video.js Web — Ad Scheduling', () => {
  ...
  test('ad pod plays all ads sequentially', async () => {
    ...
    await clickLoadContent(page);
    await page.waitForFunction(() => {
      return (document.getElementById('diagnostic-log')?.innerText ?? '').match(/ad_complete/g)?.length ?? 0 >= 3;
    }, { timeout: 120_000 });
    const log = await readDiagnosticLog(page);
...
describe('Video.js Web — VAST Compliance', () => {
  ...
  test('companion ad renders in companion slot', async () => {
    ...
    const log = await readDiagnosticLog(page);
    expect(log).toContain('companion_loaded');
  });
});
...
describe('Video.js Web — Tracking', () => {
  ...
  test('no duplicate pixels after network drop + reconnect', async () => {
    await clickLoadContent(page);
    await waitForAdContainer(page);
    await page.evaluate(() => {
      (window as Record<string, () => void>).simulateNetworkDrop?.();
    });
    await page.waitForTimeout(2_000);
    await page.evaluate(() => {
      (window as Record<string, () => void>).simulateNetworkReconnect?.();
    });
    await page.waitForTimeout(3_000);
    expect(countMatches(readCapturedUrls(), '/impression')).toBe(1);
  });
...
describe('Video.js Web — Player Integration', () => {
...
describe('Video.js Web — Viewability', () => {
...
describe('Video.js Web — Memory', () => {
  ...
  test('heap does not grow across 5 ad cycles', async () => {
